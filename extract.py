    ...
    for i in range(0, len(dirs), args.batch_size):
        ...
        try:
            with multiprocessing.Pool(4) as p:
                result = p.starmap_async(ParallelExtractDir, zip(
                    itertools.repeat(args), itertools.repeat(tmp_dir), batch_dirs))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-batch_size", "--batch_size",
                        dest="batch_size", required=False, default=3, type=int)
