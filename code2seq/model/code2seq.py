...
import torch
from seq_cross_entropy_loss import SequenceCrossEntropyLoss
...
class Code2Seq(LightningModule):
    def __init__(
        ...
        self.__loss = SequenceCrossEntropyLoss(
            pad_idx=self.__pad_idx, 
    ...
    def forward(  # type: ignore
        ...
        # [output size + 1; batch size; vocab size]
        # [output size + 1; batch size; encoder seq size]
        output_logits, attention_weights = self._decoder(
            encoded_paths, contexts_per_label, output_length, target_sequence
        )
        return output_logits, attention_weights, encoded_paths
    ...
    def _shared_step(self, batch: BatchedLabeledPathContext, step: str) -> Dict: # [_, contexts_per_label.size()]
        target_sequence = batch.labels if step == "train" else None
        # [seq length; batch size; vocab size]
        logits, _, _ = self.logits_from_batch(batch, target_sequence)
        # if logits.isnan().any():
        #     raise ValueError("Got nan predictions")
        ...
        with torch.no_grad():
            ...
            # assert torch.is_floating_point(metric.precision)
            result.update(
    ...
    def predict_step(self, batch: BatchedLabeledPathContext, batch_idx: int) -> Dict:  # type: ignore
        output_logits, attention_weights, encoded_paths = self.logits_from_batch(batch)
        return batch.labels, output_logits, attention_weights, encoded_paths, batch.contexts_per_label
