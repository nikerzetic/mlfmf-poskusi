...
class Code2Class(LightningModule):
    def __init__(self, model_config: DictConfig, optimizer_config: DictConfig, vocabulary: Vocabulary):
        ...
        metrics: Dict[str, Metric] = {
            f"{holdout}_acc": Accuracy(num_classes=len(vocabulary.label_to_id), task="multiclass") for holdout in ["train", "val", "test"] #BUG: missing task parameter
        }
    ...
    def _shared_step(self, batch: BatchedLabeledPathContext, step: str) -> Dict:
        ...
        print(logits.shape)
        labels = batch.labels.squeeze(0) #BUG: should be of size [batch size], but is of size [max label parts; batch size]
        print(labels.shape)
